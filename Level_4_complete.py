...
# Michael Beiene
# DL4H UIUC
# Level 4 Labels model for GRU(X) - GRU(Z) and tfidf(d)-atomic
# May 2022
# Version 1.0
...
def replace_with_parent_codes(string_codes, ICD9_CODE_THIRD_LEVEL, add_start_end=True):
    """replace_with_parent_codes takes a list of ICD9_CODE codes and 
    returns the list of their parents ICD9 code in the third level of the ICD9 hierarchy"""
    # adding a start and an end token to the sentence
    # so that the model know when to start and stop predicting.    
    resulting_codes = []
    for code in string_codes:
        if code[0]!='E' and code[0]!='V':
            if code[0:3] in ICD9_CODE_THIRD_LEVEL:
                resulting_codes.append(code[0:3])
    result = ' '.join(set(resulting_codes))
    if add_start_end:
        return '<start> ' + result + ' <end>' 
    else:
...
def replace_with_terminal_codes(string_codes, ICD9_CODE_FOURTH_LEVEL, add_start_end=True):
    """
    replace the labels with level_4 codes
    """
    def convert_to_ICD9(code):
        if len(code) <= 3:
            return code
        else:
    ...
    for code in string_codes:
        if code[0]!='E' and code[0]!='V':
            label = convert_to_ICD9(code)
            if label in ICD9_CODE_FOURTH_LEVEL:
                resulting_codes.append(label)
...
# ### Reading level-4 labels file

# In[11]:


with open("level_4.pkl", "rb") as f:
    ICD9_CODE_FOURTH_LEVEL = pickle.load(f)


# In[12]:


len(ICD9_CODE_FOURTH_LEVEL)
...
# ### Convert labels to Level-4 labels

# In[21]:


train_labels = [replace_with_terminal_codes(x, ICD9_CODE_FOURTH_LEVEL) for x in labels_final]
...
def evaluate_results(inputs, targets, inputs_tokenizer, target_tokenizer, max_input_length,
    ...
    y = [replace_with_terminal_codes(label, target_codes) for label in y]
...
y_true, y_pred = evaluate_results(inputs=X_test,
                                  ...
                                  target_codes=ICD9_CODE_FOURTH_LEVEL,
...
labels_b = [replace_with_terminal_codes(x, ICD9_CODE_FOURTH_LEVEL, add_start_end=False) for x in labels_final_b]
...
label_vectors, label_codes = vectorize_icd_labels(labels=labels_b,
                                                  code_list=ICD9_CODE_FOURTH_LEVEL,
...
y_test_b_pre = [replace_with_terminal_codes(x, label_codes, add_start_end=False) for x in y_test_b]
