...
# Michael Beiene
# DL4H UIUC
# Level 3 Labels model for GRU(X) - GRU(Z) and tfidf(d)-atomic
# May 2022
# Version 1.0
...
# ### Reading level-3 labels file

# In[88]:


with open("level_3.pkl", "rb") as f:
    ICD9_CODE_THIRD_LEVEL = pickle.load(f)


# In[89]:


len(ICD9_CODE_THIRD_LEVEL)
...
# ### Convert labels to Level-3 labels

# In[98]:


train_labels = [replace_with_parent_codes(x, ICD9_CODE_THIRD_LEVEL) for x in labels_final]
...
def evaluate_results(inputs, targets, inputs_tokenizer, target_tokenizer, max_input_length,
    ...
    y = [replace_with_parent_codes(label, target_codes) for label in y]
...
y_true, y_pred = evaluate_results(inputs=X_test,
                                  ...
                                  target_codes=ICD9_CODE_THIRD_LEVEL,
...
labels_b = [replace_with_parent_codes(x, ICD9_CODE_THIRD_LEVEL, add_start_end=False) for x in labels_final_b]
...
label_vectors, label_codes = vectorize_icd_labels(labels=labels_b,
                                                  code_list=ICD9_CODE_THIRD_LEVEL,
...
y_test_b_pre = [replace_with_parent_codes(x, label_codes, add_start_end=False) for x in y_test_b]
